...
class Node(asyncio.DatagramProtocol):
    def __init__(self, run_mode=REAL_MODE, configuration_file=None, log_file='logtest'):
        ...
        asyncio.ensure_future(self.__reset_every_ten_sec())
    ...
    async def __reset_every_ten_sec(self):
        while True:
    ...
    def __update_reward(self):
        distance = ((self.__dest_position_x-self.__position_x)**2 + (self.__dest_position_y-self.__position_y)**2)**0.5
        ...
        if self.__distance > distance:
            self.__reward = 1
        elif self.__distance == distance:
            self.__reward = 0
        else:
            self.__reward = -1
        self.__distance = distance
        #print("destination", self.__dest_position_x, self.__dest_position_y)
